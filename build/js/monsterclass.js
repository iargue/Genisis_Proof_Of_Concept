function monster(monster, x, y, player) {
    ...
    this.MS = monster.stats.MS * 15,
    this.RN = monster.stats.RN * 10,
    this.CMS = monster.stats.MS * 15,
    ...
    this.stageShape = new PIXI.Graphics().beginFill(monster.color).drawCircle(0, 0, 16),
    this.healthBar = new PIXI.Graphics().beginFill(0xff0000).drawRect(-18, -30, 35, 8),
    this.player = player,
	...
	this.takeDamage = function(damageAmount, damageType, attacker) {
		...
		if (this.CHP <= 0) {
			...
			gameStage.removeChild(this.stageObject);
			miniMap.removeChild(this.miniMapObject);
			if(viewTarget[1] === this){updateInfoBar('hero', activePlayer.hero)}
		} else {
			this.healthBar.clear().beginFill(0xff0000).drawRect(-18, -30, (this.CHP / this.HP) * 35, 8);
		}
	},
    ...
    this.handleCombat = function() {
        ...
        if (this.attackTarget == null) {
            ...
            if ((new Date() - this.attackTime) > this.AS) {
                ...
                if (this.alive == false) return;
                object = new PIXI.Graphics().beginFill(0xff0000).drawRect(0, 0, 4, 2);
